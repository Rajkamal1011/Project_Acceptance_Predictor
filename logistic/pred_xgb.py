...
xgbmodel = pickle.load(open('./savedModels/xgb/set2_mms_xgb_model', 'rb'))
...
def mainPredict(dict_inputs):
    ...
    predXGB = xgbPredictor(dict_newFeatures, dict_ppInputs)
    return predXGB


# Prediction Function
def xgbPredictor(dict_newFeatures, dict_ppInputs):
    ...
    xgbResult = xgbmodel.predict_proba(input_to_model)
    return xgbResult
